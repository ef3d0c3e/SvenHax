#ifndef SDK_CBASEFILESYSTEM_HPP
#define SDK_CBASEFILESYSTEM_HPP
...
MAKE_CENUM_Q(FileSystemSeek_t, int, FILESYSTEM_SEEK_HEAD, 0, // Seek relative to the start of the file
			 FILESYSTEM_SEEK_CURRENT,
			 1, // Seek relative to the current position in the file
			 FILESYSTEM_SEEK_TAIL,
			 2, // Seek relative to the end of the file
);

static constexpr int FILESYSTEM_INVALID_HANDLE = -1;

// Filesystem warning levels
MAKE_CENUM_Q(FileWarningLevel_t, int, FILESYSTEM_WARNING, -1, // A problem!
			 FILESYSTEM_WARNING_QUIET,
			 0, // Don't print anything
			 FILESYSTEM_WARNING_REPORTUNCLOSED,
			 1, // On shutdown, report names of files left unclosed
			 FILESYSTEM_WARNING_REPORTUSAGE,
			 2, // Report number of times a file was opened, closed
			 FILESYSTEM_WARNING_REPORTALLACCESSES,
			 3, // Report all open/close events to console (!slow!)
);

class CBaseFileSystem
{
public:
	virtual ~CBaseFileSystem() = 0;
	...
	/* gap, dont call */
	virtual void __CBaseFileSystem_StdioLogLevelLoadStarted(const char*) = 0;
	...
	// Does nothing.
	virtual void LogLevelLoadStarted(const char* name) = 0; // twice in the vtable because it does nothing, so the next function has the same address smh
	virtual void LogLevelLoadFinished(const char* name) = 0;

	// These 3 functions do nothing
