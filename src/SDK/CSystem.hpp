#ifndef SDK_CSYSTEM_HPP
#define SDK_CSYSTEM_HPP

#include "IBaseInterface.hpp"
struct IKeyValues;
struct KeyCode;

class CSystem : public IBaseInterface
{
public:
	virtual ~CSystem() = 0;
	...
	virtual bool __SetRegistryString(const char *key, const char *value) = 0; // idk at GetRegistryInteger there was this...
	//virtual bool GetRegistryInteger(const char *key, int &value) = 0;

	// user config
	virtual IKeyValues *GetUserConfigFileData(const char *dialogName, int dialogID) = 0;
	...
	virtual bool __CommandLineParamExists(const char *paramName) = 0; // idk
